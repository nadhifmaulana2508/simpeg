...
function ortu_preview_token() {
    ...
    return sha1(uniqid('ortu_preview_', true) . mt_rand());
}
...
try {
    ...
    if ($action === 'preview') {
        ...
        $preview_token = ortu_preview_token();
        $_SESSION['import_ortu_preview_rows'] = $json_rows;
        $_SESSION['import_ortu_preview_token'] = $preview_token;
        $_SESSION['import_ortu_preview_created_at'] = time();
        session_write_close();
        ...
        $html .= '<input type="hidden" id="import_ortu_preview_token" value="' . htmlspecialchars($preview_token, ENT_QUOTES, 'UTF-8') . '">';
    ...
    elseif ($action === 'save') {
        ...
        if ($posted_token !== '' &&
            isset($_SESSION['import_ortu_preview_token']) &&
            import_token_equals($_SESSION['import_ortu_preview_token'], $posted_token) &&
            isset($_SESSION['import_ortu_preview_rows']) &&
            is_array($_SESSION['import_ortu_preview_rows'])) {
            $data = $_SESSION['import_ortu_preview_rows'];
        } elseif (isset($_POST['data_ortu'])) {
        ...
        unset($_SESSION['import_ortu_preview_rows'], $_SESSION['import_ortu_preview_token'], $_SESSION['import_ortu_preview_created_at']);
        session_write_close();
